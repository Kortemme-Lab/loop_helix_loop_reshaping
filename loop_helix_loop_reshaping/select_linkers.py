...
def prepare_linker_selection(pose, linker_length, lhl_start, lhl_stop, front_linker):
    ...
    if front_linker:
        simple_pose_moves.insert_alas(pose, lhl_start - 1, linker_length + 2, insert_after=True, reset_fold_tree=True)
    else:
        simple_pose_moves.insert_alas(pose, lhl_start, linker_length + 2, insert_after=False, reset_fold_tree=True)
...
def select_non_clashing_linkers(pose, candidate_linkers, linker_start):
    ...
    will be reset, which means that the linker_start point is the anchor point.
    '''
